...
def Create_Robot():
    ...
    pyrosim.Send_Cube(name="Link3", pos=[x+0.5,z,y-0.5], size=[length,width,height])
    pyrosim.Send_Joint(name="Link3_Link4", parent="Link3", child="Link4", type="revolute", position=[x+1,z,y-0.5])
    pyrosim.Send_Cube(name="Link4", pos=[x+0.5,z,y-0.5], size=[length,width,height])
    pyrosim.Send_Joint(name="Link4_Link5", parent="Link4", child="Link5", type="revolute", position=[x+0.5,z,y-1])
    pyrosim.Send_Cube(name="Link5", pos=[x,z,y-1], size=[length,width,height])
    pyrosim.Send_Joint(name="Link5_Link6", parent="Link5", child="Link6", type="revolute", position=[x,z,y-1.5])
    pyrosim.Send_Cube(name="Link6", pos=[x,z,y-1], size=[length,width,height])
